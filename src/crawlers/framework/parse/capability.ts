...
export function parseCapabilityPage(
  ...
  // Headline groups: page-top callout block(s) with <p><b>label</b></p><ul>.
  const headline_groups: HeadlineGroup[] = [];
  $("div.callout-block").each((_, el) => {
    const $el = $(el);
    if ($el.find("h3").length > 0) return; // persona-page style callouts
    let current: HeadlineGroup | null = null;
    $el.children().each((_, child) => {
      const $c = $(child);
      const tag = "tagName" in child ? child.tagName.toLowerCase() : "";
      if (tag === "p" && $c.find("b, strong").length > 0) {
        current = { label: textOf($c), items: [] };
  ...
  if (kpiHeading) {
    ...
    const bulletList = section.filter("ul").add(section.find("ul")).first();
    if (bulletList.length) {
      for (const item of parseList($, bulletList)) kpi_bullets.push(item.text);
    }
  ...
  $("div.c-modal").each((_, el) => {
    ...
    if (codeBlock.length) {
      ...
      const paras: string[] = [];
      inner.children("p").each((_, p) => {
        const t = textOf($(p));
        if (t && !/^candidate data sources/i.test(t)) paras.push(t);
      });
      formula = paras.join("\n") || undefined;
      inner.find("ul li").each((_, li) => {
        const t = textOf($(li));
        if (t) data_sources.push(t);
      });
    }
