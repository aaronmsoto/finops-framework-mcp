...
5. Phase C is SPECCED and queued: `@aaronmsoto/agentic-harness@0.1.0`
   ...
   post-parity; dead protected glob removed). T-073+T-074 DONE (manifest+shim+config indirection; governance CI now npm ci --prefix .agentic with GITHUB_TOKEN + job-level packages:read, cache re-keyed — journals 20260806-t073/t074). T-075 BLOCKED (journal 20260806-t075-blocked-upstream-drift.md): npm 0.1.0 lacks the vendored fork's solo_maintainer (12665c4) and ai_attribution (T-070..072) fixes — upgrade output was a regression and was reverted, NOT committed. Unblock path: port both to agentic-starter-repo, owner publishes 0.2.0, bump .agentic/package.json, rerun T-075 then T-076. Green governance on a real PR remains the post-merge observable for the package-access grant. `dev` recreated from main. OWNER PREREQS: package access
