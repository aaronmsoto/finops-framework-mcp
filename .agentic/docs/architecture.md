...
.agentic/harness/                   Self-contained Node/TS harness (own package.json, no root deps)
...
scripts/
  agentic                  Shell shim: exec node .agentic/harness/dist/cli.js "$@"
  bootstrap.sh             cd .agentic/harness && npm ci && npm run build; install git hooks
...
Everything under `.agentic/harness/` is independent of the user's project language: a Python project deletes the root `package.json` and keeps `.agentic/harness/` intact.

## The `agentic` CLI

Invoked as `./scripts/agentic <command>` (or `node .agentic/harness/dist/cli.js`). All commands exit `0` on success, `1` on failure, `2` on usage error. `--json` on any command emits machine-readable output to stdout.
...
```jsonc
{
  "$schema": ".agentic/harness/schemas/agentic.config.schema.json",
  "preset": "self",                    // name of applied preset, or "self"/"custom"
  "project": { "name": "agentic-starter-repo", "srcDirs": [".agentic/harness/src"], "testGlobs": [".agentic/harness/tests/**"] },
...
## Harness modules (`.agentic/harness/src/`)
